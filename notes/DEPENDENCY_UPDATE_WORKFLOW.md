...
| `requirements.lock`              | Reproducible Docker/API runtime lockfile generated from `pyproject.toml` with `api`, `observability`, and `mnist` extras | `uv pip compile pyproject.toml --extra api --extra observability --extra mnist -o requirements.lock` |
