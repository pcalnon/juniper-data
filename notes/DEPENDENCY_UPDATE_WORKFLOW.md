...
| `requirements.lock`              | Reproducible Docker/API runtime lockfile generated from `pyproject.toml` with `api`, `observability`, `mnist`, and `equities` extras | `uv pip compile pyproject.toml --extra api --extra observability --extra mnist --extra equities -o requirements.lock` |
