...
class TestMackeyGlassGenerator:
    ...
    def test_euler_known_answer_from_constant_history(self) -> None:
        ...
        np.testing.assert_allclose(whole(arrays, "X")[0, :, 0], expected[:16], atol=1e-5)
        ends = np.arange(15, params.n_steps - 1)  # lookback - 1 .. T - 1 - horizon
        np.testing.assert_allclose(whole(arrays, "y")[:, 0], expected[ends + 1], atol=1e-5)
    ...
    def test_discard_shifts_start(self) -> None:
        ...
        assert whole(no_drop, "X").shape == whole(dropped, "X").shape
        assert not np.allclose(whole(no_drop, "X")[0, :, 0], whole(dropped, "X")[0, :, 0])
