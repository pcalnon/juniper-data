...
# NOTE: the equities and equities_seq fit scope is asserted END TO END in
# ``test_equities_generator.py`` (``test_normaliser_is_fit_on_train_not_full``), against the
# real conditioned-frame schema via that module's mocked-download helpers.
#
...
# executing its assertion. That is the failure mode this whole arc keeps finding, and it is
# not worth adding one deliberately for coverage of a path already covered end to end.
