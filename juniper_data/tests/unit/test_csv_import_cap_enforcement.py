...
class TestLyingStatWithOptInAnnotatesObservedSize:
    ...
    def test_under_reported_stat_does_not_persist_bytes_total_zero(self, bounded_import_dir) -> None:
        ...
        still truncates, and the annotation must not record ``bytes_total=0``
        (a complete-looking empty source). The observed read is ``cap + 1``,
        ...
        assert annotation["bytes_total"] >= TINY_CAP_BYTES + 1
        assert annotation["bytes_total"] != 0
