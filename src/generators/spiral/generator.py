...
class SpiralGenerator:
    ...
    @staticmethod
    def _generate_raw(
        params: SpiralParams, rng: np.random.Generator
    ...
    def _generate_spiral_coordinates(
        ...
        x = direction * radii * np.cos(theta) + SpiralGenerator._make_noise(
            n_points, noise, rng
        )
        y = direction * radii * np.sin(theta) + SpiralGenerator._make_noise(
            n_points, noise, rng
        )
    ...
    @staticmethod
    def _make_noise(
        n_points: int, noise: float, rng: np.random.Generator
    ) -> np.ndarray:
    ...
    @staticmethod
    def _create_one_hot_labels(
        n_spirals: int, n_points_per_spiral: int
    ) -> np.ndarray:
