...
Dataset dictionaries contain keys: `X_train`, `y_train`, `X_val`, `y_val`, `X_test`, `y_test` (all `float32`).
...
`csv_import` authorised truncation (#372, stacked on the #326 byte cap). These are not the under-cap / over-cap / 422 pins.
