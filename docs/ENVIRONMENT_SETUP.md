...
For a package version bump like `rich`, verify which file the update targets. A change under `conf/requirements*.txt` affects the setup script workflow, while a change under `pyproject.toml` affects package installation and should be followed by a refreshed `requirements.lock`.
