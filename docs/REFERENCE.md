...
| **Notify consumers** | `notify-consumers.yml` | `Publish to PyPI` completed successfully for a release; manual (re-announce a version) | Sends `repository_dispatch` `juniper-data-released` with the version to juniper-recurrence, whose bench lane then runs against exactly that release (juniper-recurrence#178). It is a separate workflow so a dispatch failure never turns a successful publish red. |
