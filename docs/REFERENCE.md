...
| `JUNIPER_DATA_CSV_IMPORT_MAX_BYTES` | int | `134217728` (128 MiB) | Deployment default byte cap. An explicitly-supplied request `max_bytes` overrides this. |
...
Above that size the binding constraint is memory, not time. `_load_csv` materialises one Python dict per row before any array exists, so raising the cap without a streaming loader trades a timeout for an OOM.

### Surfaces and precedence

| Surface | Cap (`max_bytes`) | Truncation opt-in (`allow_truncation`) |
|---------|-------------------|----------------------------------------|
| Request params (`CsvImportParams`) | Explicit value wins (`"max_bytes" in model_fields_set`) | Per-request flag |
| Environment / `.env` | `JUNIPER_DATA_CSV_IMPORT_MAX_BYTES` | `JUNIPER_DATA_CSV_IMPORT_ALLOW_TRUNCATION` |
| Compiled default | 128 MiB | `false` |

Precedence is asymmetric on purpose:

- **`max_bytes`:** an explicitly-supplied request value overrides the deployment default. Omitting the field (so the schema default is filled in) is *not* an explicit override; `_resolve_bounds` uses `model_fields_set` to tell those apart.
- **`allow_truncation`:** logical **OR**. Either the request opts in, or the deployment has opted in for every request. There is no way to opt *out* of a deployment-wide opt-in: the operator is the more privileged party.
...
1. Reads at most `cap_bytes` (`_read_capped_text`). Drops a split multi-byte UTF-8 sequence (`errors="ignore"`). Discards everything after the last newline. No newline inside the cap → empty text → existing "No data found in file".
...
| Test | What it pins |
|------|----------------|
| `tests/unit/test_csv_import_generator.py` (`TestCsvImportByteCap`) | Under-cap has no channel key; over-cap without opt-in raises `InputTooLargeError`; request and deployment opt-in annotate; explicit `max_bytes` overrides the deployment default; CSV/JSON/JSONL record boundaries; JSONL mid-file still raises; `pop_truncation_meta` returns `None` when absent |
...
| `util/ad-hoc/2026-09-04_apd_data_018_mutation_check.py` | Mutation matrix for the refusal and annotation arms |
