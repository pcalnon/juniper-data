...
| `max_bytes` | int | `134217728` (128 MiB) | Per-request cap; overrides `JUNIPER_DATA_CSV_IMPORT_MAX_BYTES` when set |
...
| `JUNIPER_DATA_CSV_IMPORT_MAX_BYTES` | int | `134217728` | `csv_import` byte cap (128 MiB) |
...
3. Raise `max_bytes` / `JUNIPER_DATA_CSV_IMPORT_MAX_BYTES` only if you also accept the memory cost — the loader is not streaming.
